...
c<- alldrag %>%
  dplyr::group_by(Month, Total_ticks) %>%
  dplyr::summarize(sumticks = n())


ggplot(data = c, aes(x = Month, y = sumticks)) +
   geom_line(color = 'black', group=2,
             data = c[!is.na(c$sumticks),],
             aes(x = Month, y = sumticks))+
  geom_point()+
  clean_background


alldrag %>%
  group_by_(.dots = c('Year', 'Month', 'Plot','Site')) %>%
  dplyr::summarize(Total_ticks = n())
...
#   dplyr::count(Total_ticks)
# 
# 
# alldrag %>%
#   dplyr::select(Year == '2016')


boop<-ggplot(data = arid, 
       aes(x = Month,y = Total_ticks),
       group = Plot, color = Plot) +
  facet_wrap(~Plot)+
...
### I LIKE THIS ONE
bleep<-ggplot(data = intermediate, 
       aes(x = Month,y = Total_ticks),
...
noises<-ggarrange(boop + rremove('xlab'),bleep + rremove('xlab'),bop+ rremove('xlab'), ncol = 3)
annotate_figure(noises,
                top = text_grob('Total Tick by Month by Site',
                                face = 'bold', size = 16,
                                color = 'red'))
...
pdf("tall.pdf", 8, 8)
par(mfrow=c(3,1))
hist(boop,xlim=c(1,12))
hist(bleep,xlim=c(1,12))
hist(bop,xlim=c(1,12))  
dev.off()
```
...
alldrag %>% group_by(Month) %>%
  summarize(amount = sum(Total_ticks))

alldrag %>%
  group_by(Month) %>%
  summarise(sum = sum(Total_ticks))
```
...
alldrag %>%
  anova_test(Total_ticks ~ Plot) # p = 8.33e-12

# look at this https://www.datanovia.com/en/lessons/ancova-in-r/
```

```{r trying again on 9/7/20}
view(alldrag)
...
p<-aov(Total_ticks ~ Plot, data = alldrag)
summary(p) # p = 8.33e-12, significant!
s<-aov(Total_ticks ~ Site, data = alldrag)
summary(s) # p < 2e-16, significant!


```


```{r tick drags by month}
### summaries by month
...
```{r try to combine sum}
newish <- alldrag %>%
  group_by(Site) %>%
  group_by(Month) 

new <-aggregate(newish$Total_ticks, 
                by = list(Month = newish$Month, Site = newish$Site, Plot = newish$Plot), FUN = sum)
colnames(new) <- c('month', 'site','plot','total_ticks')
print(new)


s <-aov(total_ticks ~ site, data = new)
summary(s) # p = 0.0177, significant!

p <-aov(total_ticks ~ plot, data = new)
summary(p) # p = 0.137, not significant


plot(y = new$total_ticks, x = new$month, data = new )
```

```{r generlized dissimilarity models GDM}
install.packages('gdm')
library(gdm)
mantel(xdis = numeric$site, ydis = numeric$total,
       method = 'pearson',
       permutation = 999)

...
aggregate(total ~ plot, data = Tick, FUN = sum)
aggregate(total ~ site, data = Tick, FUN = sum)

aggregate(total ~ month, data = Tick, FUN = sum)

(1071+600)/2692 # April and May collections
...
Basic climatic stats
```{r}
aggregate(tempdifC ~ site, data = FINAL, FUN = mean)

tempdif <- aov(tempdifC ~ site, data = FINAL)
qqPlot(tempdif$residuals)
summary(aov(tempdifC ~ site, data = FINAL)) # F value = 482.6, df =2, p-value = <2e-16 ***

Post_hoc_tempdif <-glht(tempdif, linfct = mcp(site = 'Tukey'))
summary(Post_hoc_tempdif)

?glht
confint(Post_hoc_tempdif) # pairwise test
cld(Post_hoc_tempdif) # arid = a, intermediate = b, mesic = c
plot(Post_hoc_tempdif)

aggregate(precip_mm ~ site, data = FINAL, FUN = mean)

precip <- aov(precip_mm ~ site, data = FINAL)
qqPlot(precip$residuals) # not so normal, but will argue CLT
summary(precip)

Post_hoc_precip_mm <-glht(precip, linfct = mcp(site = 'Tukey'))
summary(Post_hoc_precip_mm)
confint(Post_hoc_precip_mm) # pairwise test
cld(Post_hoc_precip_mm) # arid = a, intermediate = b, mesic = c
plot(Post_hoc_precip_mm)
...
*Best model*
BIC = 921.23
host_gnb_mod2 <- glmer.nb(total ~ liz_estimate + (1|plotID)+ (1|rain_year) + (1|month), 
                 data = na.omit(FINAL),
                 family = "poisson")
...
host_result$variables <- c("shan_mam","liz_estimates", "mam_estimates", "Full")
...
*Best fit*
BIC = 907.06
comb0_gnb_mod5 <- glmer.nb(total ~ tempdifC*site + (1|plotID)+ (1|rain_year) + (1|month), 
                 data = na.omit(FINAL),
                 family = "poisson")
BIC = 908.18                
...
**I am choosing mod3 based on the rules of parsimony**
*Interestingly, liz estimates was next best model*
BIC = 921.23
...
combo_gnb_mod7 <- glmer.nb(total ~ plot + site + tempdifC + liz_estimates + tempdifC*site + liz_estimates*plot + (1|plotID)+ (1|rain_year) + (1|month), 
                 data = na.omit(FINAL),
                 family = "poisson")
#FULL - all parameters.this was the old 8, the new 8 has interaction
# combo_gnb_mod8 <- glmer.nb(total ~ plot + site + precip_mm + tempdifC + shan_mammal + liz_estimates + tempdifC*site + liz_estimates*plot + (1|plotID)+ (1|rain_year) + (1|month), 
#                  data = na.omit(FINAL),
#                  family = "poisson")

# BIG KAHUNA
combo_gnb_mod8 <- glmer.nb(total ~ plot + site + precip_mm + tempdifC + shan_mammal + liz_estimates + tempdifC*site + liz_estimates*plot + plot*site+ (1|plotID)+ (1|rain_year) + (1|month), 
                 data = na.omit(FINAL),
                 family = "poisson")
# putting the 9 as 8
combo_gnb_mod8 <- combo_gnb_mod9

combo_result <- AIC(combo_gnb_mod1, combo_gnb_mod2, combo_gnb_mod3,combo_gnb_mod4,combo_gnb_mod5,combo_gnb_mod6,combo_gnb_mod7,combo_gnb_mod8)
combo_models <- list(combo_gnb_mod1, combo_gnb_mod2, combo_gnb_mod3,combo_gnb_mod4,combo_gnb_mod5,combo_gnb_mod6,combo_gnb_mod7,combo_gnb_mod8)
...
combo_result$variables <- c("plot", "site", "tempdifC", "liz_estimates", "tempdifC*site", "liz_estimates*plot", "Important", "Full")
...
## next best one is diff temp BIC = 908.18
```
...
```{r summ of best models}
summ(host_gnb_mod2)
summ(climate_gnb_mod3)
summ(combo_gnb_mod5)
```
...
## FULL
plot_summs(combo_gnb_mod8,scale = TRUE,
             #model.names = c("Treatment", "Aridity", "Combination"),
             inner_ci_level = .8, # width of 95CI band
             robust = TRUE)
# not sure when i leave in the stuff that is commented  out it drops 2 of my factors
# , # for standard error
#              coefs = c("Partial plot" = "plotPartial",
#                        "Total plot" = "plotTotal",
#                        "Site" = "site",
#                        "Precipitation (mm)" = "precip_mm",
#                        "Monthly temp difference (C)" = "tempdifC",
#                        "Mammal shannon diversity" = "shan_mammal",
#                        "Lizard population estimates" = "liz_estimates",
#                        "Site x Monthly temp difference" = "site:tempdifC",
#                        "Partial plot x Lizard population estimates" = "plotPartial:liz_estimate",
#                        "Total plot x Lizard population estimates" = "plotTotal:liz_estimate"),
#            dpi = 320) 
#                        #"Mammal population estimates" = "mam_estimates",
#                        #"Monthly mean max temp (C)" = "mean_maxC",
#                        #"Monthly mean min temp (C)" = "mean_minC",
#                        
#                        
#                       
#                        #"Intermediate plot" = "plotIntermediate",
#                        
#                        
#                        
#            dpi = 320) 
...
# best fit models
plot_summs(host_gnb_mod2, climate_gnb_mod3, combo_gnb_mod5,scale = TRUE,
             model.names = c("Treatment", "Aridity", "Combination"),
             inner_ci_level = .8, # width of 95CI band
             robust = TRUE)

# not sure why when i add names to coefs it drops two levels
# , # for standard error
#              coefs = c(
#                        "Lizard population estimates" = "liz_estimates",
#                        "Site" = "site",
...
```{r export summs, annotated out unless need to make updates, results = "asis"}
### export summs
  ## climate
...
delta6 <- exp(-0.5*(AIC(combo_gnb_mod8) - AIC(combo_gnb_mod5)))
delta7 <- exp(-0.5*(AIC(combo_gnb_mod7) - AIC(combo_gnb_mod5)))
delta8 <- exp(-0.5*(AIC(combo_gnb_mod8) - AIC(combo_gnb_mod5)))


# sum likelihoods
sum_likes <- sum(c(delta1, delta2, delta3, delta4, delta5, delta6, delta7, delta8))
...
weight6 <- delta6/sum_likes
weight7 <- delta7/sum_likes
weight8 <- delta8/sum_likes
...
weights <- tribble(
  ...
  "combo_gnb_mod5", round(delta5, 2), round(weight5, 2), "tempdiffC*site",
    "combo_gnb_mod6", round(delta6, 2), round(weight6, 2), "lize_estimates*plot",
    "combo_gnb_mod7", round(delta7, 2), round(weight7, 2), "important predictors",
    "combo_gnb_mod8", round(delta8, 2), round(weight8, 2), "Full") %>% 
  gt() %>% 
  tab_style(
...
# combo_gnb_mod5 has a chance of being the best model
...
Krusal-Wallis only applicable for mammals since they were sampled at 2+ sites, as well as plot for lizards

+ no significance for krusal-wallis tests

**lizards only significantly different by site type (#W = 11, p-pvalue  = 0.01306)**
  + Arid
      + mean = 71
      + median = 70
...
# lizard populations were sampled at 3 plots so a kruskal wallis test was used
...
# check normality
#qqPlot(pop_estimates$liz_estimate[site == "Arid"])
#qqPlot(pop_estimates$liz_estimate[site == "Intermediate"])
shapiro.test(pop_estimates$liz_estimate) # p > 0.05, so data are normal
leveneTest(pop_estimates$liz_estimate ~ pop_estimates$site) # p p < 0.05, F = 7.4; variances are different
...
wilcox.test(liz_estimate ~ site, data = pop_estimates) #W = 11, p-pvalue  = 0.01306


```

Aggregate data for vertebrate populations
```{r}
...
aggregate(liz_estimate ~ site, data = pop_estimates, FUN = sum)
631+1168 # 1799 lizards
aggregate(mam_estimates ~ site, data = pop_estimates, FUN = sum)
146+50+365 # 561

1799/561

aggregate(mam_estimates ~ plot, data = pop_estimates, FUN = sum) #o = 61, p = 305, total = 196
aggregate(liz_estimate ~ plot, data = pop_estimates, FUN = sum) #o = 619, p = 562, total = 618
...
**Mammal populations between exclosure and nonexclosure - just checking!**
Still not significantly different between open and total
+ but trending to differ significantly W = 11, p-value = 0.05488
```{r}

mam_sub_plot <- pop_estimates[which(pop_estimates$plot != "Partial"),]


wilcox.test(mam_estimates ~ plot, data = mam_sub_plot)

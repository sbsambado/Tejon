...
#View(climate)
climate$Month <- factor(climate$Month, 
...
plot(y = log(climate$Total_ticks),x =  climate$precip..mm.)
#plot(glm.nb(Total_ticks ~ precip..mm., data = climate, family = 'poisson'))


aggregateticks <- climate %>%
  group_by(Site) %>%
  group_by(Month) 

ggplot(data = climate, aes(y = aggregateticks$Total_ticks,x =  aggregateticks$precip..mm., group = Site, color = Site))+
  geom_smooth()

ggplot(data = climate, aes(y = precip..mm.,x =  Site,))+
  geom_point()


rain <- aggregate(precip..mm. ~ Site + Total_ticks + Year, FUN = sum, data = climate)
plot(rain)

ggplot(data = rain, aes(y = log(precip..mm.),x =  Site, color = Year))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(size = 3, alpha = .5) +
  theme_bw()

ggplot(data = rain, aes(x = log(precip..mm.),y =  log(Total_ticks), color = Year))+
  geom_jitter(size = 2, alpha = .5) +
  theme_bw()

mod <- glm(Total_ticks ~ precip..mm., family = 'poisson', data = rain)

plot(predictorEffects(mod))

mod2 <- glm(precip..mm. ~ Site, family = 'poisson', data = rain)

plot(predictorEffects(mod2))

rain$Year <- as.factor(rain$Year)
mod3 <- glm(precip..mm. ~ Year, family = 'poisson', data = rain)
plot(predictorEffects(mod3))

ggplot(data = rain, aes(y = log(precip..mm.),x =  Year, color = Site))+
  geom_boxplot(outlier.shape = NA)+
...
ggplot(data = rain, aes(y = precip..mm.,x =  Year, color = Site))+
  geom_boxplot(outlier.shape = NA)+
  theme_bw()

rain_in <- aggregate(precip..in. ~ Site + Total_ticks + Year, FUN = sum, data = climate)
rain_in$Year <- as.factor(rain_in$Year)

ggplot(data = rain_in, aes(y = log(precip..in.),x =  Year, color = Site))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(size = 3, alpha = .4) +
  theme_bw()
...
ggplot(data = rain_in, aes(y = precip..in.,x =  Year, color = Site))+
  geom_boxplot(outlier.shape = NA)+
  theme_bw()


rain2 <- aggregate(precip..mm. ~ Site + Total_ticks + Year + Month, FUN = sum, data = climate)
plot(rain2)

ggplot(data = rain2, aes(y = log(precip..mm.),x =  Site, color = Year))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(size = 3, alpha = .5) +
  theme_bw()

ggplot(data = rain2, aes(x = log(precip..mm.),y =  log(Total_ticks), color = Year))+
  geom_jitter(size = 2, alpha = .5) +
  theme_bw()

mod3 <- glm(Total_ticks ~ precip..mm., family = 'poisson', data = rain2)

plot(predictorEffects(mod3))

mod4 <- glm(Total_ticks ~ precip..mm. + Month, family = 'poisson', data = rain2)

plot(predictorEffects(mod3))


ggplot(data = rain2, aes(y = precip..mm.,x =  Month, color = Year))+
  geom_jitter(size = 2, alpha = .5) +
  theme_bw()+
...
ggplot(data = rain2, aes(y = log(precip..mm.),x =  Month, color = Year))+
  geom_boxplot() +
  geom_point()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = 0.5))
...
ggplot(data = rain2, aes(y = log(precip..mm.),x =  Month, color = Site))+
  geom_boxplot() +
  geom_point()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = 0.5))
...
ggplot(data = rain2, aes(y = precip..mm.,x =  Month, color = Site))+
  geom_point()+
  theme_bw()+
  facet_wrap(~ Year)+
  theme(axis.text.x = element_text(angle = 90, size = 8, vjust = 0.5))
...
summary(m1 <- glm.nb(Total_ticks ~  Site, data = climate))
summary(m2 <- glm.nb(Total_ticks ~  Year, data = climate))
summary(m3 <- glm.nb(Total_ticks ~  diff, data = climate))
summary(m4 <- glm.nb(Total_ticks ~  precip..mm., data = climate))
summary(m5 <- glm.nb(Total_ticks ~  mean_max_F, data = climate))
summary(m6 <- glm.nb(Total_ticks ~  Site + mean_max_F, data = climate))
summary(m7 <- glm.nb(Total_ticks ~  Year + mean_max_F, data = climate))
summary(m8 <- glm.nb(Total_ticks ~  diff + mean_max_F, data = climate))
summary(m9 <- glm.nb(Total_ticks ~  diff + precip..mm., data = climate))

View(climate)
# what hillary wanted
Tick_abundance ~ climate + treatment + climate*treatment + 1|plotID + 1|month+ 1|rainfall year
summary(m10 <- glm(Total_ticks ~ mean_max_F + Plot + mean_max_F*Plot + 1|Block + 1|Month+ 1|rain_year, data = climate))
...
plot(predictorEffects(m6))
plot(predictorEffects(m7))
plot(predictorEffects(m8))
plot(predictorEffects(m9))
...
~~~~~~~~~~~~~~~~~
predictor effects of glm
```{r}
library(effects)

# example from Poissonregressionexample
#plot(predictorEffects(mod.ornstein, xlevels=list(assets=100)),
#    axes=list(x=list(rotate=40)))

plot(predictorEffects(tick1a))

plot(predictorEffects(tick5))



sinclair.full <- glm(COUNT~SEX*MARROW*DEATH,family=poisson, data=sinclair)
sinclair.full_nointeraction <- glm(COUNT~SEX+MARROW+DEATH,family=poisson, data=sinclair)
...
```{r}
library(lme4)
glmer(Total_ticks ~ climate + treatment  + 1|month + 1|rainfall year, family = 'poisson' ,
     data = climate)
plot(glmer(Total_ticks ~ Site + 1|Month, family = 'poisson' ,
     data = climate))

glm(Total_ticks ~ diff  + Site + diff*Site + 1|Month + 1|rain_year, family = 'poisson',data = climate)


cor.test(climate$diff, climate$Total_ticks)
...
**on 10/14/2020 I added precipitation data and added diff column**
...
**Mesic : Bear Valley Springs collected by NOAA**

+ ID : KCATHEHAC23
+ Lat, Long : 35.04, 118.57
+ **Elevation: 1455.42 m**
...
# mean max vs tot tick count
ggplot(data = climate, aes(x = mean_max_F, y = Total_ticks, group = Site, color = Site))+
  geom_smooth()+
  clean_background+
...
ggplot(data = climate, aes(y = diff, x = Site))+
  geom_boxplot()+
  clean_background +
  xlim('Arid', 'Intermediate', 'Mesic')


ggplot(data = climate, aes(y = mean_max_F, x = Site))+
  geom_boxplot()+
  clean_background +
  xlim('Arid', 'Intermediate', 'Mesic')

ggplot(data = climate, aes(y = mean_min_F, x = Site))+
  geom_boxplot()+
  clean_background +
  xlim('Arid', 'Intermediate', 'Mesic')

#ggplot(climate, aes(diff, color = Site)) + 
...
best single variable (lowest AIC score of 10313.06) : **diff**

best multivariable variable (lowest AIC score) : **diff**


```{r glms with temp}

tick1a <- glm(Total_ticks ~ diff, 
               data = climate,
             family = 'poisson')

tick1b <- glm(Total_ticks ~ mean_max_F, 
               data = climate,
             family = 'poisson')

tick1c <- glm(Total_ticks ~ mean_min_F, 
               data = climate,
             family = 'poisson')

tick1 <- glm(Total_ticks ~ rain_year, 
               data = climate,
             family = 'poisson')
...
tick5 <- glm(Total_ticks ~ Site + rain_year, 
               data = climate,
              family = 'poisson')

tick6 <- glm(Total_ticks ~ Site + rain_year + Plot, 
               data = climate,
              family = 'poisson')

tick7 <- glm(Total_ticks ~ Site + rain_year + Plot + Month, 
               data = climate,
              family = 'poisson')
...
tickresults <- AIC(tick1a,tick1b,tick1c, 
                   tick1, tick2, tick3,
                    tick4, tick5, tick6,
                   tick7)

# add other metrics to table
tickmodels <- list(tick1a,tick1b,tick1c,
                   tick1, tick2, tick3,
                    tick4, tick5, tick6,
...
tickresults$variables <- c('diff', 'mean_max', 'mean_min','rain_year','Site', 'Plot', 'Month', 'Site + rain_year', 'Site + rain_year + Plot', 'Site + rain_year + Plot + Month')
...
fldown <- formula(Total_ticks ~ diff + Site + diff*Site + 1|Month + 1|rain_year,
                  data = climate) # what explains the load that's not 0 | explanatory variables if something is present or not
...
glm(Total_ticks ~ diff, data = climate, family = 'poisson')
glm(Total_ticks ~ mean_max_F, data = climate, family = 'poisson')
glm(Total_ticks ~ mean_min_F, data = climate, family = 'poisson')
...
ggplot(data = Aggregateticks,
  ...
  ggtitle('Total tick abundance by site and plot')+
  theme(plot.title = element_text(face = 'bold', hjust = 0.5)) 
ggsave("TotalticksSitePlotYear.pdf", dpi = 320)
```
